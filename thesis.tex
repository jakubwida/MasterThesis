...
The number of CUDA threads is created, for every voxel in \textbf{the voxel array}. For every voxel, the Cieśla's algorithm is executed against every figure in the neighborhood, in which lies the voxel. If any figure causes the voxel to be rejected, it is marked as such on \textbf{the voxel array}. \newline
