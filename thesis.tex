...
The voxel is rejected if no new shapes can be inserted within it, at any position or angle. This happens if there exists at least one shape in the packing, that would overlap any new polydisk within the voxel. It is enough that one circle of these pre-existing polydisks would overlap any new circle from a virtually inserted shape \cite{ciesla}. \newline \newline
...
The goal of this thesis was to create a GPU-parallel implementation of the algorithm described in the previous section. The algorithm was supposed to generate shapes in the same way as the sequential implementation, but with shorter execution times. \newline
