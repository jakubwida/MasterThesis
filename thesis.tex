...
The previously studied shapes include rectangles, squares and other polygons, as well as cubes and hypercubes. A commonly studied shape are disks and spheres, and their equivalents in up to eight dimensions. Other studies involved spheroids, hyperdisks and disk polymers. In case of non spherical shapes, the shape angle can be randomly generated \cite{zhang}.
...
A significant example is the Poisson Disk Sampling. It is a type of random distribution of points within space. The generated points are no closer to each other, than a given distance. The Poisson Disk Sampling can be simulated with the use of RSA algorithms using circles or spheres. \newline
...
The previously described algorithm holds a basic structure, which is used in the expanded implementations. However, there are added several features in order to improve it's functionality and execution time. Also, to enable the use of polydisks, rather than simple circles in the packing. These features are listed below.
...
\begin{figure}[H]
  \centering
	...
	\caption{Packing after rejection of shapes against the pre-existing ones. Only small groupings of new shapes remain. The same as in \ref{GPURSA_Process_1} the pre-existing shapes are red and green. The new polydisks are blue.}
...
Outside of theoretical inquiries, the actual perforance was investigated. All calculations were performed using the 8GB RAM / Intel® Core i5-8250U CPU @ 1.60GHz × 8 / Nvidia® GeForce MX150 / Ubuntu 18.04 laptop. The GPU had 384 CUDA cores operating at a 1708Mhz clock and 2048 MB of memory. The processor has a 6 MB memory cache. \newline
...
If the proportions of shapes that were not succesfully inserted is above the voxel split treshold, then they are split. The added shape number comes in multiplies of 512 in order to effectively utilise the 512 threads in CUDA block.
