...
The algorithm proposed by Michał Cieśla covers the solution to this problem. \newline
...
$(x,y,\alpha)$ \newline
Where $x,y$ are it's carthesian position, while $\alpha$ represent's its angular position. \newline
...
where: \newline
$d_x(f_x,f_\alpha)=[x+f_x*\delta r + R_i * cos(\alpha_i+f_alpha * \delta \alpha)-x_0]^2$ \newline
$d_y(f_y,f_\alpha)=[x+f_y*\delta r + R_i * sin(\alpha_i+f_alpha * \delta \alpha)-y_0$ \newline
...
\textbr{TODO UNFINSHED - finish the equations}
